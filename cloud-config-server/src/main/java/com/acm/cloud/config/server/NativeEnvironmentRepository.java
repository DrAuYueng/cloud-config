...
/**
 * Simple implementation of {@link EnvironmentRepository} that uses a
 * SpringApplication and configuration files located through the normal
 * protocols. The resulting Environment is composed of property sources located
 * using the application name as the config file stem (spring.config.name) and
 * the environment name as a Spring profile.
 * 
 * @author Dave Syer
 * @author Roy Clarkson
 */
...
public class NativeEnvironmentRepository implements EnvironmentRepository {
    ...
    /**
     * Locations to search for configuration files. Defaults to the same as a
     * Spring Boot app so
    ...
    /**
     * Flag to determine how to handle exceptions during decryption (default
     * false).
     */
    ...
    public Environment findOne(String config, String profile, String label) {
        ...
        // Explicitly set the listeners (to exclude logging listener which would
        // change
    ...
    private ConfigurableEnvironment getEnvironment(String profile) {
        ...
        environment.getPropertySources().addFirst(
                new MapPropertySource("profiles", Collections.<String, Object> singletonMap("spring.profiles.active", profile)));
        return environment;
    }
    ...
    protected Environment clean(Environment value) {
        ...
        for (PropertySource source : value.getPropertySources()) {
            ...
            if (searchLocations != null) {
                ...
                if (!matches) {
                    // Don't include this one: it wasn't matched by our search
                    // locations
